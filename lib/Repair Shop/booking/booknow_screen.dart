...
class _CarRepairBookingScreenState extends State<CarRepairBookingScreen> {
  ...
  final _barangayController = TextEditingController();  // Added Barangay controller
  final _cityController = TextEditingController();
  final _provinceController = TextEditingController(); // Changed state to province
  ...
  Future<void> requestLocationPermission() async {
    ...
    if (status.isGranted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Location permission is denied. Please enable it to proceed."))
      );
  ...
  Future<void> _getAddressFromCoordinates(double latitude, double longitude) async {
    try {
      ...
      setState(() {
        ...
        _barangayController.text = place.subLocality ?? 'Unknown barangay';  // Added Barangay field
        _cityController.text = place.locality ?? 'Unknown city';
        _provinceController.text = place.administrativeArea ?? 'Unknown province'; // Changed to province
        _zipCodeController.text = place.postalCode ?? 'Unknown ZIP code'; // Populate ZIP code
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (value) {
                    setState(() {
                      ...
                      _selectedPrice = issues.firstWhere((issue) => issue['name'] == value)['price'].toString();
                    });
                  },
                  decoration: const InputDecoration(
                    border: OutlineInputBorder(),
                  ...
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      if (_selectedIssue == null) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text("Please select an issue description.")),
                        );
                        return;
                      }
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => BookingDetailsScreen(
                            ...
                            barangay: _barangayController.text, 
                            province: _provinceController.text, 
                            serviceProviderName: '',  // Pass service provider info if available
                            serviceProviderProfile: '', 
                            isUrgent: true, 
